...
- **Title Row**: Row 2 is reserved for the title. Explicitly set row height to prevent clipping: `ws.row_dimensions[2].height = 30` (adjust upwards if font size requires).
...
# ws['B2'].fill = title_fill  # FORBIDDEN
```



### Visual Themes
#### 1. Minimalist Monochrome Style (Default)
...
header_dark_grey = "333333"   # Section headers
border_grey = "D0D0D0"        # Standard borders
blue_primary = "0066CC"       # Primary Accent
...
bg_light = "ECF0F1"           # General background
header_dark_blue = "1F4E79"   # Data Header fill
accent_warm = "FFF3E0"        # Key metrics highlight
negative_red = "FF0000"       # Standard negative (International)

# Application Example: Data Headers
gs_header_fill = PatternFill(start_color=header_dark_blue, end_color=header_dark_blue, fill_type="solid")
gs_header_font = Font(name='Times New Roman', color="FFFFFF", bold=True)
...
header_fill = PatternFill(start_color=header_dark_grey, end_color=header_dark_grey, fill_type="solid")
header_font = Font(name='Times New Roman', color="FFFFFF", bold=True)
for cell in ws['B3:E3'][0]:  # Data headers, not title row
    cell.fill = header_fill
    cell.font = header_font
```
